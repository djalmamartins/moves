...
namespace Source\Public\Studio;
...
{
    private const VIEW_PATH = __DIR__ . "/../../../container/studio/moves_studio/";
    ...
    public function login(?array $data): void
    {
        ...
        echo $this->view->render("components/login/login", [
            "head" => $this->seo->render(
                ...
                CONF_SITE_DESC,
                url("/studio/login"),
                themeStudio("/assets/images/favicon.png", "moves_studio")
            ),
    ...
    public function settings(?array $data): void
    {
        ...
        if (!empty($data["action"])) {
            ...
            $studioTemplates = $this->templateDirectories(dirname(__DIR__, 3) . "/container/studio");
            foreach (["view_theme" => $siteThemes, "view_support" => $supportThemes, "view_app" => array_values(array_filter($studioTemplates, fn($name) => str_starts_with($name, "app_"))), "view_erp" => array_values(array_filter($studioTemplates, fn($name) => $name !== "moves_studio" && !str_starts_with($name, "app_")))] as $field => $available) {
        ...
        echo $this->view->render("components/settings/home", $this->viewData("Configurações", "settings", $user, [
            ...
            "appThemes" => array_values(array_filter($this->templateDirectories(dirname(__DIR__, 3) . "/container/studio"), fn($name) => str_starts_with($name, "app_"))),
            "erpThemes" => array_values(array_filter($this->templateDirectories(dirname(__DIR__, 3) . "/container/studio"), fn($name) => $name !== "moves_studio" && !str_starts_with($name, "app_")))
