...
/**
 * ERP | Class Model Layer Supertype Pattern
 *
...
abstract class Model
{
    ...
    public function fetch(bool $all = false)
    {
        try {
            ...
            $this->fail = $exception;
            AppLogger::exception($exception, 'database', ['event_type' => 'model_query_failed', 'entity' => $this->entity]);
            return null;
        }
    }
    ...
    protected function create(array $data): ?int
    {
        try {
            ...
            $this->fail = $exception;
            AppLogger::exception($exception, 'database', ['event_type' => 'model_create_failed', 'entity' => $this->entity]);
            return null;
        }
    }
    ...
    protected function update(array $data, string $terms, string $params): ?int
    {
        try {
            ...
            $this->fail = $exception;
            AppLogger::exception($exception, 'database', ['event_type' => 'model_update_failed', 'entity' => $this->entity]);
            return null;
        }
    }
    ...
    public function save(): bool
    {
        ...
        if (!empty($this->id)) {
            $id = $this->id;
            $before = (clone $this)->findById((int)$id);
            $this->update($this->safe(), "id = :id", "id={$id}");
            if ($this->fail()) {
                $this->message->error("Erro ao atualizar, verifique os dados");
                return false;
            }
            Audit::record("update", $this->entity, $id, (array)($before ? $before->data() : []), (array)$this->data());
        }
        ...
        if (empty($this->id)) {
            ...
            Audit::record("create", $this->entity, $id, [], (array)$this->data());
        }
    ...
    public function userLastId(): int
    {
        return Connect::getInstance()->query("SELECT MAX(id) as maxId FROM {$this->entity}")->fetch()->maxId;
    }

    public function idLast(): int
    {
        return Connect::getInstance()->query("SELECT MAX(id) as maxId FROM {$this->entity}")->fetch()->maxId;
    }
    ...
    public function delete(string $terms, ?string $params): bool
    {
        try {
            ...
            $this->fail = $exception;
            AppLogger::exception($exception, 'database', ['event_type' => 'model_delete_failed', 'entity' => $this->entity]);
            return false;
        }
    }
    ...
    public function destroy(): bool
    {
        ...
        if ($destroy) {
            Audit::record("delete", $this->entity, $this->id, (array)$this->data(), []);
        }
        return $destroy;
    }
