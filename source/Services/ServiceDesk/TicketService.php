...
final class TicketService
{
    ...
    {
        $terms=[];$params=[];$status=(string)($filters['status']??'');$priority=(string)($filters['priority']??'');$assigned=(string)($filters['assigned_to']??'');$due=(string)($filters['due']??'');$q=trim(strip_tags((string)($filters['q']??'')));
        if(in_array($status,['open','in_progress','waiting_customer','resolved','closed'],true)){$terms[]='t.status=:status';$params['status']=$status;}if(in_array($priority,['low','medium','high','urgent'],true)){$terms[]='t.priority=:priority';$params['priority']=$priority;}if($assigned==='unassigned')$terms[]='t.assigned_to IS NULL';elseif(ctype_digit($assigned)&&(int)$assigned>0){$terms[]='t.assigned_to=:assigned';$params['assigned']=(int)$assigned;}if($due==='today')$terms[]='DATE(t.due_at)=CURDATE()';elseif($due==='overdue')$terms[]="t.due_at<NOW() AND t.status NOT IN ('resolved','closed')";if($q!==''){$terms[]="(t.subject LIKE :q OR t.protocol LIKE :q OR t.requester_name LIKE :q OR t.requester_email LIKE :q OR CONCAT(r.first_name,' ',r.last_name) LIKE :q)";$params['q']="%{$q}%";}
        $sql="SELECT t.*,c.name condominium_name,d.protocol demand_protocol,COALESCE(NULLIF(TRIM(CONCAT(r.first_name,' ',r.last_name)),''),t.requester_name,'Solicitante') requester_name,CONCAT(a.first_name,' ',a.last_name) assigned_name FROM studio_support_tickets t LEFT JOIN users r ON r.id=t.requester_id LEFT JOIN users a ON a.id=t.assigned_to LEFT JOIN operation_condominiums c ON c.id=t.condominium_id LEFT JOIN operation_demands d ON d.id=t.demand_id".($terms?' WHERE '.implode(' AND ',$terms):'')." ORDER BY FIELD(t.status,'open','in_progress','waiting_customer','resolved','closed'),FIELD(t.priority,'urgent','high','medium','low'),t.created_at DESC LIMIT 100";$stmt=$this->pdo->prepare($sql);$stmt->execute($params);return$stmt->fetchAll()?:[];
    }
