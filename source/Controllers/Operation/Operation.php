...
class Operation extends Controller
{
    ...
    {
        $user=$this->operationUser('operation.agenda.manage'); $pdo=Connect::getInstance(); $agenda=new AgendaService($pdo);
        if($_SERVER['REQUEST_METHOD']==='POST'){
            ...
            $id=(int)($data['event_id']??0); if(($data['action']??'')==='delete'&&$id){$agenda->delete($id);echo json_encode(['redirect'=>url('/operation/agenda')]);return;}
            try{$id=$agenda->save($data??[],(int)$user->id);$this->logActivity($pdo,'agenda',$id,'saved',(string)($data['title']??'Compromisso'),(int)$user->id);echo json_encode(['redirect'=>url('/operation/agenda')]);}catch(\InvalidArgumentException $exception){http_response_code(422);echo json_encode(['message'=>$this->message->warning($exception->getMessage())->render()]);}return;
        }
        $type=in_array($_GET['type']??'',['meeting','task','deadline','support','visit'],true)?$_GET['type']:'';$filters=$_GET;$filters['type']=$type==='visit'?'':$type;$calendar=$agenda->events($filters);$events=$type==='visit'?[]:$calendar['events'];$status=$calendar['status'];$assigned=$calendar['assigned'];$condo=$calendar['condominium'];
        if($type===''||$type==='visit'){$visitTerms=["(v.scheduled_at>=:from OR (v.recurrence_rule IS NOT NULL AND v.recurrence_rule<>''))","v.scheduled_at<:to"]; $visitParams=['from'=>$calendar['from'],'to'=>$calendar['to']];if($status){$visitTerms[]='v.status=:status';$visitParams['status']=$status;}if($assigned){$visitTerms[]='v.assigned_to=:assigned';$visitParams['assigned']=$assigned;}if($condo){$visitTerms[]='v.condominium_id=:condo';$visitParams['condo']=$condo;}$visits=$pdo->prepare("SELECT v.id,v.condominium_id,v.title,v.objective description,v.scheduled_at starts_at,v.ends_at,v.recurrence_rule,NULL reminder_minutes,c.address location,'visit' type,v.status,v.assigned_to,CONCAT(u.first_name,' ',u.last_name) assigned_name,c.name condominium_name,c.latitude,c.longitude,'visit' source_type,0 editable FROM operation_visits v LEFT JOIN users u ON u.id=v.assigned_to JOIN operation_condominiums c ON c.id=v.condominium_id WHERE ".implode(' AND ',$visitTerms));$visits->execute($visitParams);$events=array_merge($events,$visits->fetchAll()?:[]);}
        $events=$this->expandCalendarRecurrences($events,$calendar['from'],$calendar['to']);usort($events,static fn($a,$b)=>strcmp($a->starts_at,$b->starts_at));$view=in_array($_GET['view']??'month',['day','week','month','list','map'],true)?$_GET['view']:'month';
        echo $this->view->render('components/agenda/home',$this->operationView('Agenda','agenda',$user,['month'=>$calendar['month'],'viewMode'=>$view,'events'=>$events,'users'=>(new User())->find("status != :status","status=trash")->order('first_name,last_name')->fetch(true)?:[],'condominiums'=>$pdo->query("SELECT id,name FROM operation_condominiums WHERE status<>'inactive' ORDER BY name")->fetchAll()?:[],'typeFilter'=>$type,'statusFilter'=>$status,'assignedFilter'=>$assigned,'condominiumFilter'=>$condo]));
    }
