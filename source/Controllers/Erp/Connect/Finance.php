...
class Finance extends Erp
{
    ...
    public function entries(?array $data): void
    {
        ...
        if($_SERVER['REQUEST_METHOD']==='POST'){
            ...
            try{$action=(string)($data['action']??'create');$entryId=(int)($data['entry_id']??0);if($action==='cancel'){if(!$service->cancel($condominiumId,$entryId))throw new \InvalidArgumentException('Lançamento não pode ser cancelado.');}elseif($action==='pay'){$service->pay($condominiumId,$entryId,(string)($data['payment_amount']??''),$data,(int)$this->user->id);}elseif($action==='reconcile'){$service->reconcile($condominiumId,(int)($data['transaction_id']??0),$entryId,(int)$this->user->id);}elseif($action==='ignore_transaction'){if(!$service->ignoreTransaction($condominiumId,(int)($data['transaction_id']??0)))throw new \InvalidArgumentException('Transação não pode ser ignorada.');}elseif($action==='update'){if(!$service->updateEntry($condominiumId,$entryId,$data))throw new \InvalidArgumentException('Lançamento não pode ser editado.');}else{$data['condominium_id']=$condominiumId;$service->createEntry($data,(int)$this->user->id);}echo json_encode(['redirect'=>url('/erp/finance/entries')]);}catch(\InvalidArgumentException $exception){http_response_code(422);echo json_encode(['message'=>$this->message->warning($exception->getMessage())->render()]);}return;
        }
        ...
        echo $this->view->render('components/finance/entries',['app'=>'finance/entries','head'=>$head,'items'=>$items,'transactions'=>$condominiumId?$service->bankTransactions($condominiumId):[],'totals'=>$totals,'filters'=>$filters,'edit'=>$edit,'condo'=>(object)['select'=>$this->condo,'list'=>(new AppCondominium())->find()->fetch(true)]]);
    }
