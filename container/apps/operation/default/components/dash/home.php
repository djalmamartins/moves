...
$dashboardKpis = [['Demandas abertas',(int)($demandsOpen??0),'warning-outline','/operation/demandas'],['Chamados abertos',(int)($ticketsOpen??0),'headset-outline','/operation/chamados'],['Visitas hoje',$totalToday,'calendar-outline','/operation/visitas'],['Tarefas atrasadas',(int)($tasksOverdue??0),'time-outline','/operation/meu-dia'],['Orçamentos pendentes',(int)($quotesPending??0),'calculator-outline','/operation/orcamentos'],['Pendências críticas',(int)($criticalIssues??0),'alert-circle-outline','/operation/issues']];
?>
...
<section class="operation-dashboard-summary">
  <article class="operation-next-card">
    ...
    <div><small>Próxima visita</small><h2><?= htmlspecialchars($nextEvent->title ?? 'Agenda livre') ?></h2><p><ion-icon name="calendar-outline"></ion-icon><?= htmlspecialchars($nextEvent ? $nextEvent->time . ' · ' . $nextEvent->condominium_name : 'Nenhuma visita programada para hoje') ?></p><span><?= $nextEvent?'Programação operacional confirmada':'Use a agenda para planejar o dia' ?></span></div>
    <div class="operation-next-actions"><a class="operation-btn primary" href="<?= url($nextEvent->source_url ?? '/operation/agenda') ?>"><ion-icon name="play-outline"></ion-icon><?= $nextEvent?'Abrir visita':'Agendar horário' ?></a><a class="operation-btn" href="<?= url('/operation/agenda') ?>">Ver agenda</a></div>
  </article>
...
<section class="operation-dashboard-grid">
  ...
  <article class="operation-dashboard-panel operation-day-list"><header><div><h2>Meu dia</h2><p>Visitas e atividades programadas.</p></div><a href="<?= url('/operation/meu-dia') ?>">Ver todos</a></header><div><?php if($dayAgenda): foreach($dayAgenda as $event): ?><a href="<?= url($event->source_url) ?>"><time><?= htmlspecialchars($event->time) ?></time><i><ion-icon name="calendar-clear-outline"></ion-icon></i><span><strong><?= htmlspecialchars($event->title) ?></strong><small><?= htmlspecialchars($event->condominium_name . ($event->description ? ' · ' . $event->description : '')) ?></small></span><ion-icon name="chevron-forward-outline"></ion-icon></a><?php endforeach; else: ?><div class="operation-empty"><ion-icon name="calendar-clear-outline"></ion-icon><strong>Nenhuma visita hoje</strong><small>Seu dia está livre neste momento.</small></div><?php endif; ?></div><footer><a class="operation-btn full" href="<?= url('/operation/agenda') ?>"><ion-icon name="add-outline"></ion-icon>Nova visita</a></footer></article>
